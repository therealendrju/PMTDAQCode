...
sql_interface::sql_interface(char * DBname)
{
	//static MYSQL *conn;
	...
	//cout << *hostnamet <<" t " << endl;
	std::string hostname(hostnamet);
	char server[20];

	sprintf(server,"warp02.lngs.infn.it");
	std::cout << hostname << std::endl;
	if(hostname=="warp02.lngs.infn.it")
	...
	fConn = (MYSQL *)mysql_init(NULL);

	/* Connect to database */
	if (!mysql_real_connect(fConn, server,
	     user, password, database, 0, NULL, 0)) {
		     fprintf(stderr, "%s\n", mysql_error(fConn));
		     std::cout << "Could not connect to the database, please make sure it is on";
		     std::cout << "log in as root on warp02 and execute: /etc/init.d/mysqld start " << std::endl;
		     std::exit(0);
	     }
...
int sql_interface::get_pmt_sers(double * sers,int * onoff,int nofpmts,int nrun,int vscale)
{
//return 0;
...
if(!vscale || vscale==50)
	...
	sprintf(query," ");
	sprintf(query,"SELECT pmt_b,pmt_a,sers.onflag FROM runs, sers WHERE sers.run_nr=runs.run_nr AND sers.run_nr = %d ORDER BY pmt",nrun );
...
int sql_interface::get_pmt_sigmas(double * sigmas,int nofpmts,int nrun)
{
//return 0;
...
int sql_interface::get_pmt_results(results * sers,int * onoff,int nofpmts,int nrun)
{
...
int sql_interface::get_pmt_sers_with_vscale(double * sers,int * onoff,int nofpmts,int nrun,int vscale)
{
...
///////////////////////////////////
int sql_interface::get_ser_parameter_results(int nrun,float *pars)
{
...
int sql_interface::insert_pmt_sers(double * sers,int *onoff,int nofpmts,int nrun,int flag,int scale)
{
...
int sql_interface::insert_pmt_sers(results * sers,int * onoff,int nofpmts,int nrun,int flag,int scale)
{
//sers array must be filled with nofpmts+1 fields. I.e. 4 pmts = double[5]
// flag is 0 for before run, 1 for after run.
...
int sql_interface::insert_new_run(int nrun,int nofpmts,int scale)
{
	int ser_id;
...
int sql_interface::get_last_ser_entry()
{
...
// int sql_interface::get_last_param_entry()
// {
// 
// MYSQL_RES *res;
// MYSQL_ROW row;
...
int sql_interface::insert_new_sers_entry(int ser_id,int nofpmts)
{
	char query[300];
...
int sql_interface::insert_new_ser_parameters_entry(int run_id)
{
	char query[300];
...
int sql_interface::check_ser_entry_exists(int nrun,int * ser_id)
{
...
int sql_interface::check_ser_parameters_exists(int nrun)
{
...
int sql_interface::check_run_entry_exists(int nrun,int * ser_id)
{
	MYSQL_RES *res;
	MYSQL_ROW row;
	int exists=0;
...
int  sql_interface::insert_ser_parameters(int runin,float threshold,int integr_window, int chan_before, int chan_after)
{
	//MYSQL_RES *res;
	//MYSQL_ROW row;
	int exists=0;
